...
public class TarefaController {
	...
	@GetMapping
	public ResponseEntity<List<ListagemTarefaDTO>> getAll(){
		List<ListagemTarefaDTO> tarefas = tarefaService.getAll();
		if(!tarefas.isEmpty())
	...
	public ResponseEntity<TarefaDTO> saveTarefa(@RequestBody TarefaDTO tarefaDTO) {
		return new ResponseEntity<>(tarefaService.saveTarefa(tarefaDTO), HttpStatus.CREATED);
	}
	
	@PutMapping("/{id}")
	public ResponseEntity<TarefaDTO> updateTarefa(@PathVariable Integer id, @RequestBody TarefaDTO tarefaDTO) {
		TarefaDTO tarefaAtualizada = tarefaService.updateTarefa(id, tarefaDTO);
